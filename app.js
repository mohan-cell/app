...
function parseM3U(data) {
    ...
    for (let line of lines) {
        ...
        if (line.startsWith('#EXTINF:')) {
            // Highly robust regex to catch groups and logos regardless of quotes or spacing
            ...
            // Push a hard copy to ensure channels don't accidentally mix data
...
searchInput.addEventListener('input', (e) => {
    ...
    channels.forEach((channel, index) => {
        ...
        // Only show if the exact term is found in name or group
...
// Keep UI open when interacting with search
searchInput.addEventListener('focus', showUI);

// Prevent TV remote keys from executing while typing
...
document.addEventListener('keydown', e => {
    if (document.activeElement === searchInput) return; // Ignore if user is typing
